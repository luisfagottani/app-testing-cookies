...
/**
 * Lê `referral_id` da query e, quando `syncFromQuery` é true (padrão), grava cookie
 * com `Domain=.agottani.dev` em hosts agottani.dev — subdomínios compartilham o valor.
 * Com `syncFromQuery: false` apenas lê (útil neste projeto em auth.agottani.dev).
 * Em localhost não usa Domain (cookie só vale na origem atual).
 */
export function useReferralCookie(
  paramName = 'referral_id',
  options: { maxAgeDays?: number; syncFromQuery?: boolean } = {},
): UseReferralCookieResult {
  const maxAgeDays = options.maxAgeDays ?? 30;
  const syncFromQuery = options.syncFromQuery ?? true;
  ...
  useEffect(() => {
    ...
    if (syncFromQuery && fromQuery) {
      ...
      setCapturedFromUrl(false);
    } else {
      setReferralIdInCookie(null);
      setCapturedFromUrl(false);
    }
  }, [fromQuery, maxAgeDays, syncFromQuery]);
